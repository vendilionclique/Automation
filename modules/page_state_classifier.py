...
def _request_payload(model: str, image_path: Path, keyword: str, temperature: float) -> Dict[str, Any]:
    ...
    prompt = (
        ...
        "upper-right corner. This state has priority over visible_ready when a normal homepage is visible "
        "behind the overlay but the popup covers or intercepts the homepage search box, search button, "
        "results controls, or ordinary page interaction. Common examples include Taobao red-packet, coupon, "
        "subsidy, receive-now, and expiring-benefit popups. Do not use closeable_popup_overlay for login, "
        "captcha, security/risk, account, "
