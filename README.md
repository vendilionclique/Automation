...
# 默认示例使用付费高速 GLM-4.6V-FlashX；编辑本机文件，只填写 MIDSCENE_MODEL_API_KEY。
```
...
bounded capture。GLM-4.6V-FlashX 后默认采集主线是 bounded act 业务边界：
`desktop_chrome_ready_boundary` 先恢复/确认既有 Chrome 采集窗口，采集主干再按
`home_entry_boundary` -> `search_submit_boundary` -> `capture_tiles_boundary` 推进；
普通营销遮挡只走 `safe_popup_repair_boundary`，登录、验证码、风控、权限面板、未知高风险
或任何隐藏数据需求统一进入 `human_stop_boundary`。Midscene/外部 VLM
只在可见屏幕上推进这些边界内的连续 GUI 动作；每个关键词都必须先从淘宝首页或
