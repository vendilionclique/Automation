...
bounded capture。FlashX 后默认采集主线是 bounded act 搜索/滚动：Midscene/外部 VLM
只在可见屏幕上推进搜索框定位、输入提交、等待和滚动；Python 负责截图、粗页面状态判断、
保存 viewport tiles、异常停机和 watchdog 自恢复。环境不可用时写 `real_not_available`，
不会伪装成已采集。
...
3. 用 Midscene computer bounded act 完成首页搜索、首屏截图和滚动截图，确认 Python 能保存 tile 并识别异常状态。
