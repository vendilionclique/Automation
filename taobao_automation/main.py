...
class TaobaoAutomation:
    ...
    def run_batch(self, excel_file, resume=False):
        ...
        # 分析超时
        analysis_timeout = self.config.getint('PLUGIN', 'analysis_timeout', 120)
        ...
        for i, keyword in enumerate(keywords):
            ...
            try:
                ...
                if success:
                    # 导出结果（从DOM读取）
                    export_result = self.exporter.export_results(keyword)
    ...
    def _finalize(self):
        ...
        merged_file = merge_filtered_results(self.filtered_dir,
