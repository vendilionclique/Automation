...
7. The capture worker owns session-level screenshot capture. In v1 `python harness.py visual-capture-worker --contract <...>` is a framework/simulation entry; future real capture must stay within Midscene computer pure-vision boundaries.
8. The extract worker owns screenshot-to-rows persistence. `python harness.py visual-extract-worker --plan-id <plan_id> --session <N>` consumes captured keyword screenshots, writes extract artifacts, reuses ingest/export, and deletes successful screenshots only at keyword granularity.
...
12. Keep DB/LLM/statistical/final assignment decoupled from the capture and extract workers.

## Double Worker Memory

- Current architecture is four-layer: local deterministic heartbeat, session-level capture worker, asynchronous extract worker, and non-resident Codex supervisor.
...
- Extract worker reads captured screenshots, writes `rows_pending.json`, `extract_result.json`, `extract_worker_result.json`, and raw rows via existing ingest/export. It must not operate the browser.
...
- The same sync scripts create a Codex profile named `taobao_visual_cron` with `model = "gpt-5.5"`, `sandbox_mode = "danger-full-access"`, and `approval_policy = "never"`. Run Taobao visual cron jobs with that profile, or with an equivalent app-level setting that allows process enumeration, GUI foregrounding, and filesystem screenshot persistence.
