...
- For large project development, use a subagent team by default. The main agent should coordinate, divide work, review, integrate, and report; bounded implementation, local exploration, test-fix work, small follow-up fixes, verification, and patch cleanup should be delegated to worker/explorer subagents whenever the current environment supports them. Prefer reusing existing subagents that still have enough context instead of spending the main agent's context. Only keep work in the main agent when no subagent can be created or reused, when reuse overhead clearly exceeds the task, or when the human explicitly requests a single agent.
- If context is running low, compression looks unreliable, or a GPT-5.5 context-compaction stall is suspected, save progress into durable project files before continuing: session capsule files, `summary.json`, `events.jsonl`, `task_events.jsonl`, `control.json`, and/or `visual_tasks.json`. Ask the human to continue in a fresh thread when needed. Fresh threads must read `AGENTS.md` and the durable files instead of relying on old chat history. Codex App updates may reduce this issue later, but file state remains authoritative.
...
- Extract dispatch also passes `approval_policy` and `sandbox_mode` as command-level `codex exec -c` overrides and uses `--ignore-rules` by default to reduce execpolicy rules impact. Keep extract prompts self-contained; short-lived workers should be instructed not to read AGENTS.md, skills, README, project docs, or repository rules.
...
- Midscene Zhipu VLM defaults should prefer the 4.6v vision model for long-running capture stability. Do not configure model fallback for Taobao capture. Treat 429 errors containing `余额不足或无可用资源包`, `无可用资源包`, `code: 1113`, or `code 1113` as `vlm_resource_unavailable`; stop the current session/keyword for supervisor pause/cooldown instead of retrying as ordinary QPS throttling.
- To reduce VLM calls, use the Midscene MCP `Scroll` tool with a fixed distance between viewport tiles. Keep `act` available for visible search-box grounding and only use it for scrolling if fixed `Scroll` fails with a non-resource error.
