...
- 已将 tracked 默认 Midscene grounding 模型从 `glm-4.6v-flashx` 改为 `glm-4.6v-flash`，包括
  `config/settings.example.ini`、`local/midscene-computer.env.example`、Codex sync 脚本、
  README/docs 和相关测试。`glm-4.6v-flash` 可作为免费开发/备用模型，但真实 8 关键词
  session 复测显示它会返回 `429 该模型当前访问量过大，请您稍后再试`，不能作为正式
  无人值守采集主线 SLA。
...
- `harness.py visual-capture-watchdog --plan-id <plan_id> --session N --start`
  ...
  不加 `--start` 时仅 dry-run/advice。`visual-automation-tick --start-capture` 保留为
  兼容薄入口，但不再作为文档主线。
- 当前真实测试显示 `glm-4.6v` 可用于 Midscene grounding，但单次 VLM 响应约 10-55 秒，
  会显著拉长 session；测试阶段可继续使用，生产阶段应按稳定性和速度再评估付费更快 API。
- 真实测试中 `ClearInput` 在结果页搜索框上出现失焦风险，表现为页面级全选/清空倾向。
  后续搜索框替换策略应增加 fallback：点击稳定搜索框坐标后优先键盘 `Cmd+A`、输入、
  回车并截图断言；`ClearInput` 可保留但失败一次即停，不做反复清空。
...
- 搜索和滚动 prompt 已补强：搜索框填入关键词后必须按 Enter 或点击搜索按钮提交；
  如果 Codex、Terminal、Cursor、VS Code 或其他 app 在前台，先切回既有 Chrome
  采集窗口，不得把关键词或滚动动作打到非 Chrome 应用。
