...
- 当前本机依赖自检已通过：`.venv/bin/python3 harness.py setup` 全绿；`pandas`、`openpyxl`、`Pillow`、`pyperclip` 均可导入。
...
5. Codex/视觉自动化层使用本机 Chrome 真实登录态，低频、可暂停地打开淘宝首页并从可见搜索框搜索
6. 通过 pure-vision action 或系统级截图采集商品列表可视区域截图；每个可见窗口截图视为一个 viewport tile
7. Codex 从批量 tile 截图识别商品标题、价格、店铺、地区等字段，轻量去重后给 raw 行写入 `采集时间`
...
- v1 采用“批量截图 + 一次性识别 + 跨 tile 轻量去重”，稀疏早停和逐 tile 动态识别留作后续优化。
