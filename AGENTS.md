...
- 已将 tracked 默认 Midscene grounding 模型切到 Z.AI GLM-5V-Turbo
  `glm-5v-turbo`，包括 `config/settings.example.ini`、
...
- 历史真实测试显示 GLM-V 可用于 Midscene grounding；当前默认已切到
  GLM-5V-Turbo `glm-5v-turbo`。thinking/reasoning 默认关闭，避免实时 grounding 变慢。
- 真实测试中 `ClearInput` 在结果页搜索框上出现失焦风险，表现为页面级全选/清空倾向。
  GLM-5V-Turbo 后采集主线已改为 bounded act 搜索/滚动，由 prompt 约束可见搜索框定位、输入、
...
- 搜索框关键词确认规则已调整为更严格的关键词边界策略：每个新关键词必须先从淘宝首页或已确认的
  ...
  结果页。GLM-5V-Turbo/page-state probe 读出旧关键词、读不清且没有强证据，或画面仍是旧结果页时，
...
- 不恢复“点搜索框、全选、复制、读剪贴板”的默认主线；也不把 `Tap`、`Input`、
  ...
  bounded `act`，Python 只用截图/page-state probe 做验收与停机决策。
...
- 旧结果页/到底页的 home-entry repair 已明确允许一条可见 UI 路径：配置
  ...
  一个 Chrome 标签页时才可关闭。
- Midscene grounding 主线已切到 Z.AI GLM-5V-Turbo：`model_name=glm-5v-turbo`、
  `base_url=https://api.z.ai/api/paas/v4`、`model_family=glm-v`，thinking/reasoning 默认关闭。
...
未来可能拆出员工版轻量采集端，但不作为当前开发日程。员工版不包含 DB/SSH/后处理/最终赋值资产，只负责使用本机 Chrome 登录态和付费高速 GLM-5V-Turbo / 视觉自动化 Agent 采集可见结果，输出标准化 `raw_results.xlsx`、`raw_rows.jsonl`、截图证据和 manifest；thinking/reasoning 默认关闭。
