...
- 已将 tracked 默认 Midscene grounding 模型切回付费高速 GLM-4.6V-FlashX
  ...
  `MIDSCENE_MODEL_REASONING_ENABLED=false`，`MIDSCENE_MODEL_TEMPERATURE=0`。Kimi/Moonshot
  已放弃，不作为主线；GLM-5V-Turbo 仅作为后续 A/B 评估候选。
...
- 历史真实测试显示 GLM-V 可用于 Midscene grounding；当前默认已切回付费高速
  GLM-4.6V-FlashX `glm-4.6v-flashx`。Kimi/Moonshot 已放弃，不作为采集主线；
  GLM-5V-Turbo 仅作为后续 A/B 评估候选。
- 真实测试中 `ClearInput` 在结果页搜索框上出现失焦风险，表现为页面级全选/清空倾向。
  FlashX 后采集主线已改为 bounded act 搜索/滚动，由 prompt 约束可见搜索框定位、输入、
  提交、等待和滚动；`Tap`、`Input`、`KeyboardPress`、`Scroll` 等短动作工具只保留为
  调试/人工修正/后续 fallback 能力，不再作为默认采集路线。
- capture worker 已接入 Midscene/VLM coarse page-state probe：当
  `allow_midscene_page_state_probe=true` 且 MCP `assert` 可用时，`tile_00` 和后续
  tile 截图后优先让 Midscene 基于当前可见截图判断有限 operational state
  (`visible_ready` / `visible_results` / `search_results` / `results_page` /
  `empty_result` / `login_required` / `captcha_required` / `risk_suspected` /
  `white_skeleton` / `popup_blocked` / `unknown`)。本地 `page_state.py` 的颜色/区域
  heuristic 只作为 probe 关闭、不可用、超时或无法解析时的 fallback；`unknown`、
  登录、验证码、风控、白屏和弹窗仍必须进入 `needs_review`，不能为跑通 session
  自动标记 `captured`。
- Midscene page-state probe 的边界是“粗页面状态”，不输出商品标题、价格、店铺、
  价格可信度、业务过滤或统计决策；商品字段仍只由 Codex extract worker 基于保留的
  可见截图抽取，再由 `visual-apply-extracted-rows` 确定性落盘。
- bounded act 搜索和滚动 prompt 已补强：搜索框填入关键词后必须按 Enter 或点击搜索
  按钮提交；如果 Codex、Terminal、Cursor、VS Code 或其他 app 在前台，先切回既有
...
- Midscene MCP 预批准面已收窄为主线必要工具：`computer_connect`、`take_screenshot`、
  `act`、`assert` 与 display/list/connect 辅助；`Tap`、`Input`、`KeyboardPress`、
...
- Midscene/VLM 仍按无状态 API 使用；每次 `act` / `assert` 只依赖当前 prompt 和当前可见
  屏幕。因此 prompt 已补强：淘宝底部不要求固定 `1/100` 这类页码分母，也不要求出现
...
- 搜索框关键词确认规则已调整为关键词边界策略：新搜索后的 `tile_00` / post-act 验收仍必须
  证明当前关键词已经进入可采集结果页；如果 FlashX/page-state probe 读出旧关键词或读不清，
  capture worker 不再立即要求人工介入，而是用一次 bounded act reset/retry 关闭或离开旧结果
  页，找到已有淘宝首页或打开淘宝首页，从全新的空搜索框重新搜索当前关键词，再重新验收。重置
  后仍读不清、读错或遇到异常时才进入 `needs_review` / cooldown。
...
- `results_end` 现在按“当前关键词已经到底”处理：即使底部搜索框关键词读不清或读出疑似旧关键词，
  ...
  当前关键词，不 reset/retry 后把重搜首屏当作完整采集。reset/retry 只用于新关键词
  `tile_00` / post-act 验收失败；失败截图副本（例如 `tile_00_initial_failed.png`）必须保留。
  硬异常如登录、验证码、风控、弹窗、白屏、限流等不能被 reset 掩盖，仍进入人工复核/冷却。
- 不恢复“点搜索框、全选、复制、读剪贴板”的默认主线；也不把 `Tap`、`Input`、
  `KeyboardPress`、`ClearInput` 等短动作接回 unattended capture。搜索和 reset/retry 仍由
  bounded `act` 负责，Python 只用截图/page-state probe 做验收与停机决策。
...
未来可能拆出员工版轻量采集端，但不作为当前开发日程。员工版不包含 DB/SSH/后处理/最终赋值资产，只负责使用本机 Chrome 登录态和付费高速 GLM-4.6V-FlashX / 视觉自动化 Agent 采集可见结果，输出标准化 `raw_results.xlsx`、`raw_rows.jsonl`、截图证据和 manifest；Kimi/Moonshot 不作为主线。
...
1. **真实 Midscene 留样复核**
   - 连接本机 Chrome 登录态，低频执行单关键词采集。
   - 从淘宝首页可见搜索框输入关键词并触发搜索，保存首屏和滚动分屏截图。
   - 被动记录淘宝登录弹窗、验证码、安全验证、风险提示、白屏骨架、空结果/错误页等真实返回文案。
   - 确认 capture worker 的异常分类命中；未命中的真实文案补充到分类器和回归测试。
   - 继续审计确认没有调用 DOM/HTML/network/storage/JS eval 读取能力。

2. **单 session 视觉闭环复测**
   - 运行 `python3 harness.py visual-plan-day --raw-input cards.xlsx --random-sample 1 --session-count 1` 创建小型 daily plan。
