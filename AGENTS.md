...
- `harness.py visual-codex-extract-dispatch --plan-id <plan_id> --session N`
  ...
  dispatch 命令必须显式带上 `CODEX_EXTRACT.approval_policy` / `sandbox_mode`
  的 `-c` 覆盖，默认使用 `--ignore-rules` 减少 execpolicy rules 影响，并在
  summary 输出 `effective_extract_config`。extract prompt 必须自包含，并要求
  worker 不读取项目规则文件。旧 `launch_state.json` 的死亡 PID 要标记为
  `stale` / `previous_pid_dead` 后允许重派。
...
- 2026-05-15 稳定性修正：Midscene 智谱 VLM 默认示例改用 4.6v 视觉模型
  (`glm-4.6v`，如智谱后台实际 id 不同以本机配置调整)，淘宝采集不做模型
  fallback。429 中包含 `余额不足或无可用资源包`、`无可用资源包`、
  `code: 1113` 或 `code 1113` 时按 `vlm_resource_unavailable` 立即停止当前
  session/keyword 并交由 supervisor 暂停/冷却，不按普通 QPS 三次重试。tile
  滚动优先使用固定距离 Midscene MCP `Scroll` 工具，失败且非资源包错误时才考虑
  `act`。
...
- 大项目开发默认采用 subagent team：主 agent 负责需求拆解、上下文分发、任务调度、
  代码审查、集成和最终汇报；具体实现、局部排查、测试修复等工作交给边界清晰的
  worker/explorer subagent。主 agent 不应在大项目里同时承担主要开发者角色，除非
  当前环境无法创建或复用 subagent、任务非常小，或用户明确要求单 agent 完成。小修、
  验证、补丁收尾等也应优先复用仍有上下文余量的已有 subagent，而不是让主 agent
  消耗自己的上下文；只有没有可用 subagent 或复用成本明显高于任务本身时才由主 agent
  直接处理。
- 当上下文接近耗尽、压缩可能影响可靠性，或出现 GPT-5.5 上下文压缩卡死迹象时，
  立即把进度保存到项目文件而不是继续依赖聊天历史。优先更新 session capsule
  (`session_request.json` / `session_prompt.md` / `summary.json` / `events.jsonl`) 或
  任务级 `task_events.jsonl`、`control.json`、`visual_tasks.json`，并让用户开启/切换到
  新线程继续。新线程必须读取这些项目文件和本 `AGENTS.md`，继续遵循 subagent team
  规则。随着 Codex App 后续更新，上下文压缩卡死问题可能会缓解，但项目仍以可恢复
  文件状态作为权威来源。
