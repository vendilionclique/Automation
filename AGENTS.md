...
- 当前代码级自检：`.venv/bin/python -m py_compile harness.py modules/*.py` 通过；
  `pandas`、`openpyxl`、`Pillow` 均可导入。本机
...
- `post_keyword_cleanup` 是交给 Midscene `act` 的可见 UI prompt，不是 Python 短动作驾驶代码；
  cleanup 只作为已成功关键词后的诊断/清场尝试，不能替代下一个关键词自己的 pre-entry 验收。
  不再固定要求 `Command+W` / `Ctrl+W`；Midscene 应优先选择低风险可见 UI 路径离开当前结果页。
...
- 近期性能判断已固定为优先降本项：当前付费 FlashX 成本不只来自每张 tile 的
  `page_state_classifier`，也可能来自 Midscene bounded `act` 内部的多轮 VLM grounding。
  Midscene Web/XPath/DOM 类缓存不适合淘宝纯视觉主线，不能为了省钱打开 DOM/XPath/selector
  缓存；下一步应改为项目侧可审计的低频视觉策略：关键词入口仍由 bounded `act` 完成，
  `tile_00` 仍做硬验收，结果页稳定后尽量用 Python 控制的固定滚动/系统截图/相似度早停推进，
  只在首屏、疑似到底、疑似异常、周期性抽检或 cheap check 不确定时调用 VLM classifier。
  同时必须为 Midscene `act` 和 JSON classifier 记录 `started_at`、`finished_at`、`elapsed_ms`、
  阶段名和调用次数，先用账本确认钱烧在 grounding 还是 page-state classifier，再决定是否切换
  Qwen3-VL 等本地/低价模型。
...
- 当前业务边界集合不是只有“三段”：`desktop_chrome_ready_boundary` 先恢复/确认既有
  Chrome 采集窗口，采集主干按 `home_entry_boundary` -> `search_submit_boundary` ->
  `capture_tiles_boundary` 推进；普通营销遮挡只走 `safe_popup_repair_boundary`，登录、
  验证码、风控、权限面板、未知高风险或任何隐藏数据需求统一进入 `human_stop_boundary`。
- 2026-05-17 接棒收口后，采集主干业务流程固定为：关键词开始前先确认淘宝首页/普通搜索入口；从首页搜索框提交当前关键词；`tile_00` 必须证明商品列表页属于当前关键词；滚动采集可见 tiles，遇到到底或相邻可采集 tile 高度相似则结束当前关键词；商品字段抽取仍在 capture 之后由 Codex extract worker 处理。
...
- 当前主线默认关闭详情页、购物车/收藏等“更像真人浏览”的额外动作；
  `detail_page_peek_probability=0`、`allow_cart_or_favorites_peek=false`、
  `cart_or_favorites_peek_probability=0` 只是保留远期实验占位。未来若恢复鼠标自然移动、输入退格、
  首页推荐流商品随机打开/滚动/关闭、商品详情页、购物车或收藏页等行为模拟，必须作为单独实验重新设计
  prompt、行为预算、账号安全 stop rule 和测试，不能静默接回默认采集主线。
...
1. **VLM / Midscene 交互降本改造**
   - 先加成本与耗时账本：按阶段记录每次 Midscene `act` 调用、JSON classifier 调用、
     `classifier_started_at`、`classifier_finished_at`、`elapsed_ms`、成功/超时/限流和 fallback 原因。
   - 保持 `tile_00` 硬验收不降级；后续稳定结果页滚动优先改成 Python 固定距离滚动、系统截图、
     相邻截图相似度和 cheap image check，减少每个 tile 都让 Midscene 重新理解页面的次数。
   - 设计低频 classifier 策略：首屏必问，疑似到底/异常必问，普通中间 tile 可抽检或在 cheap check
     不确定时再问；classifier 超时只能保留截图并标 `classifier_timeout/unknown` 或
     `captured_partial`，不能让 heuristic 的 `visible_ready` 参与业务验收。
   - 不启用 Midscene Web/XPath/DOM 缓存作为淘宝主线；如需像素坐标复用，只能做项目侧可审计的
     屏幕几何/稳定区域复用，并保留异常 stop rule。
