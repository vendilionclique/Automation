...
- `harness.py visual-session-run <plan_id> --session N` also writes a bounded
  ...
  screenshot review, `visual-ingest`, filtering, and downstream assignment.
...
- 2026-05-14 架构已调整为“四层职责”：本地无脑 heartbeat、session 级
  capture worker、异步 extract worker、非常驻 Codex supervisor。Codex 不再作为
...
- `harness.py visual-capture-worker --contract <midscene_session_worker_request.json>`
  是采集 worker 框架；v1 只读取 contract 并写标准 `keyword_result.json` /
  `session_worker_result.json`，真实 Midscene 连续执行后续接入。
- `harness.py visual-extract-worker --plan-id <plan_id> --session N` 是抽取 worker
  框架；它消费 captured keyword 的截图，写 `rows_pending.json` /
  `extract_result.json` / `extract_worker_result.json`，复用现有 ingest/export。
...
- 当前代码级自检：`.venv/bin/python -m py_compile harness.py modules/*.py` 通过；
  ...
  `[MIDSCENE_COMPUTER] session_keyword_limit`，否则 `harness.py setup` 会提示失败。
...
7. extract worker 异步消费 captured keyword 的批量 tile 截图，抽取商品标题、价格、
   店铺、地区等字段，轻量去重后给 raw 行写入 `采集时间`
...
- heartbeat 必须短命、可重复运行、可从文件恢复；不要把 Codex/聊天 agent 会话
  当作长期在线主调度器
- capture worker 和 extract worker 通过文件、lease/runtime 和事件日志解耦；Codex
...
- extract worker 不允许“单张图抽完即删”；必须等同一 keyword 完成 ingest 且质量
...
- v1 采用“capture worker 批量截图 + extract worker keyword 级抽取/去重/ingest”；
...
python3 harness.py visual-extract-worker --plan-id <plan_id> --session 1

python3 harness.py visual-one 中止
python3 harness.py visual-run <run_id> --limit 1
python3 harness.py visual-ingest data/tasks/<run_id> --keyword "万智牌 中止" --rows-file rows.json
